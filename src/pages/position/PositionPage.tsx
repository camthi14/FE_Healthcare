...
import FormAddEditDepartment from "./form/FormAddEditPosition";
import { positionActions, usePositions } from "~/features/position";
...
const PositionPage: FC = () => {
  ...
  const columns = useMemo(
    (): ColumnTable[] => [
      ...
      { id: "name", label: "Tên chức vụ", minWidth: 170 },
  ...
  const initialValues = useMemo((): IPosition => {
  ...
  const texts = useMemo(() => {
    if (isEditMode) return { title: `Cập nhật chức vụ`, textBtn: "Lưu thay đổi" };
    return { title: `Thêm chức vụ`, textBtn: "Thêm mới" };
  }, [isEditMode]);

  const handleSubmit = useCallback(
    (values: IPosition, resetForm: () => void) => {
      if (isEditMode) {
        return dispatch(
          positionActions.addEditStart({ type: "edit", data: values, resetData: resetForm })
        );
      }

      dispatch(positionActions.addEditStart({ type: "add", data: values, resetData: resetForm }));
    },
    [isEditMode]
  );

  const handleSelected = useCallback((mode: ModeTypes, item: IPosition) => {
    // @ts-ignore
  ...
  const textRemove = useMemo(() => {
    ...
    return {
      ...
      contentText: `Tên chức vụ: \`${selected.data?.name}\``,
    };
  }, [selected.type, selected.data]);
  ...
  const onAgreeRemove = useCallback(() => {
    ...
    dispatch(positionActions.deleteStart(`${selected.data.id}`));
  ...
  const handleOnChangePage = useCallback(
    (newPage: number) => {
      dispatch(positionActions.setFilter({ ...filters, page: newPage }));
    },
    [filters]
  );
  ...
  const handleChangeName = useCallback(
    (value: string) => {
      setName(value);
      dispatch(
        positionActions.setDebounceSearch({
          limit: filters.limit,
          page: filters.page,
          name_like: value,
        })
